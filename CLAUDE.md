...
- **05_developer_orchestration — DONE (deterministic TDD pipeline; NOT a questionnaire/loop phase).** This is Phase 2, so it has its own global prompt `workflows/active-build-phase.md` (Phase 2 tree only). The unit of work is one feature's `documentation/features/<feat>/scope.md` (from PM). **Core principle: agents write code + a status report; `khazad-dum` is the deterministic part** — it runs the project's `test_commands` (from `.khazad-dum/config.json`) as the real gate, owns all git, and drives every transition + escalation. Seven `*-phase.md` prompts, titled `Workflow 2.1.x`, all tier-agnostic (NEVER name a model/effort — `khazad-dum` sets `--model`/`--effort`): `design-phase` (2.1.1, skeleton w/ `todo!()`), `red-phase-e2e` (2.1.2, e2e tests in `e2e_tests` crate), `red-phase-unit` (2.1.3, co-located unit tests), `red-review-phase` (2.1.4, certainty-scores the tests vs scope's e2e requirements), `red-edit-phase` (2.1.5, fixes flagged tests), `green-phase` (2.1.6, production code to pass all tests), `green-diagnosis-phase` (2.1.7, runs when escalation exhausted → verdict `test-defect|scope-defect|impl-hard`). Pipeline: design→red-e2e→red-unit→[red-review↔red-edit loop]→green→(on exhaustion) diagnosis. **Green-blocked routing = diagnose + bounded kickback:** top-tier green failure → diagnosis; `test-defect` kicks back to red-edit **once**, else human. **Escalation ladder (config, not prompts):** haiku/medium → sonnet/medium → opus/high → human. Templates (3): `status-report.md` (all phases), `review-report.md` (2.1.4 certainty+deficiencies), `diagnosis-report.md` (2.1.7 verdict) — each a machine-readable ```json block + prose, written to `.khazad-dum/orchestration_log/<feature>/`. `refactor.md` left in place but is **stale** (it's an old "set up cargo workspace" prompt, not a TDD refactor) — handle when/if a real refactor phase is added.
...
- Update `cli._SUB_PROMPT_TEMPLATES`: research gate needs `questionnaire` + `literature-review`; experiment gate needs `questionnaire`; architecture loop needs `questionnaire` + `c4-diagram` + `ddd-table` + `skeleton-proposal`; architecture complete needs `detailed-project-description`; PM loop needs `questionnaire` + `feature-scopes` + `scope`. Step-05 templates: every 2.1.x phase needs `status-report` except 2.1.4 (needs `review-report`) and 2.1.7 (needs `diagnosis-report`); each phase also gets the selected feature's `scope.md` injected as its scope.
- **Step-05 TDD orchestration engine (the big new deferred chunk).** `khazad-dum` runs each feature's `scope.md` through a deterministic state machine over the 2.1.x prompts. After each agent run it executes the project's `test_commands` (config) and routes on the *real* result (NOT the agent's self-report):
  - Per-feature: checkout branch (git) → 2.1.1 design (gate: `cargo build` compiles) → 2.1.2 red-e2e + 2.1.3 red-unit (gate: tests build + new tests FAIL + existing still pass) → 2.1.4 red-review (gate: parse `certainty` from `review-report`; ≥ threshold → green, else → 2.1.5 red-edit → back to 2.1.4, bounded by a review-loop guard) → 2.1.6 green (gate: ALL tests pass + clippy + fmt).
  ...
  - All thresholds/budgets/loop-guards/tier ladder live in `.khazad-dum/config.json`. Reports + logs go to `.khazad-dum/orchestration_log/<feature>/<workflow-id>-<slug>-{N}.md` ({N} = attempt/cycle, harness-injected). `init` should create `orchestration_log/` (or the orchestrator does on first run).
...
- `01_research`–`05_developer_orchestration` templates are complete; phase 06 still has a 3-line `templates/template.md` stub. Experiment + architecture + PM still carry an unused stub `templates/template.md` (the active scope seed until `steps.py` gets their `sub_prompts`, after which `project-description.md` takes over). Step 05's old `prompts/refactor.md` is **stale** ("set up cargo workspace", not a TDD refactor) — left in place, not wired into the 2.1.x pipeline.
